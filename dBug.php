...
class dBug
{
	...
	function getVariableName()
	{
		...
		for ($i = count($arrBacktrace) - 1; $i >= 0; $i--) {
			...
			if (is_array($arrCurrent) && array_key_exists("function", $arrCurrent) && isset($arrCurrent["function"]) && in_array($arrCurrent["function"], $arrInclude))
				continue;
			$arrFile = $arrCurrent;
			break;
		}
		...
		return !empty($arrMatches[1]) ? $arrMatches[1] : null;
	}
